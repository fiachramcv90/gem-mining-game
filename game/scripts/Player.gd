...
func _account_fuel() -> void:
	...
	if below_now > below_prev:
		...
		GameState.drain_fuel((below_prev - below_now) * eco.fuel_ascent_per_tile \
				* Upgrades.ascent_factor())
...
func _draw() -> void:
	...
	if mine != null and mine.is_breakable(_drill_cell):
		...
		draw_rect(Rect2(center - Vector2(px, px) * 0.5, Vector2(px, px)), Color(1, 1, 1, 0.16), true)
		var need := drill_time_for(_drill_cell)
		if need > 0.0:
			var frac := clampf(_drill_progress / need, 0.0, 1.0)
			draw_arc(center, px * 0.42, -PI / 2, -PI / 2 + TAU * frac, 20,
					Color(1.0, 0.85, 0.3), 2.0)
	# Body + facing nose.
