#!/usr/bin/env python3
"""Generate the PWA icons for the iOS smoke-test harness (wayfinder ticket 0011).

Pure-stdlib PNG writer (zlib only) so it runs anywhere with no Pillow/ImageMagick.
Draws a dark rounded square with a cyan gem/diamond and a small "S" mark so the
home-screen icon is recognisable as the *smoke test* (distinct from the 0004
prototype). THROWAWAY — regenerate by running `python3 gen_icons.py`.
...
GEM = (0x5e, 0xc8, 0xe0)     # cyan
GEM_HI = (0x8f, 0xe0, 0xf0)  # highlight
GEM_LO = (0x3f, 0x9c, 0xb8)  # shadow
