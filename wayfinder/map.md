...
- [Art & audio direction](tickets/0008-art-audio.md) — **make everything, spend nothing.** 16 px, one fixed master palette **Resurrect-64** with the load-bearing **reserve-saturation** rule (rock desaturated; gems/lava/prize-glint own the saturated hues → readable at the edge of the shrinking view radius); bands = hue+value shift of a shared rock ramp; **halo telegraph = darker/tighter ring**; prize = a gold cross-glint shader piercing darkness; lava self-lit. **All free/hand-made** (Pixelorama + procedural noise-dither; AI/Claude scoped to palette/reference/code-gen, *not* final sprites). **Near-zero hand-drawn frames** — motion via `Tween`s + **capped pooled particles** + **one reused CanvasItem shader**. Sound is a **bonus layer** (silent switch mutes it): SFX via jsfxr/ChipTone + foley dig-thud, *"no bus effects" is runtime-only so bake FX into samples offline*; music = self-composed **depth-crossfaded ambient loops** (Bosca Ceoil). **Juice is visual-first** — every beat lands with sound off; "haptics-equivalent" = screen-shake+flash; reduce-motion toggle + best-effort `vibrate`. [Direction note](assets/0008-art-audio.md) · [moodboard](assets/0008-palette-moodboard.html).
- [On-device iOS Safari smoke test](tickets/0011-ios-smoke-test.md) — **0002 CONFIRMED on a real iPhone — all four legs.** A throwaway single-threaded Compatibility/WebGL2 export runs in iOS **WebKit**: renders (`gl_compatibility` / OpenGL ES 3.0 / WebGL 2.0), touch works, **Sample audio unlocks on first tap** (once the silent switch is off), **PWA installs (Add to Home Screen)**, holds **60 FPS**, and — the headline for 0002 §4 — **survived 9 rotate/resizes incl. landscape with no canvas-resize crash and no context loss.** No blocker → platform bet stands (not re-litigated). Two forward notes: **(a) the iOS ring/silent switch mutes Web Audio** — real UX consideration for [0008](tickets/0008-art-audio.md)/onboarding (players on silent hear nothing); **(b) on-device memory numbers didn't instrument** (Godot `MEMORY_STATIC` reads 0 on web; the WASM-heap shim didn't capture `WebAssembly.Memory`), so the memory result is *qualitative* and the profiling task must fix web memory instrumentation first. Build bug caught en route: `OS.get_current_rendering_method()` doesn't exist in 4.3 → parse error → silent grey screen; now a **CI headless-run gate** fails builds on any script error. [Learning notes](assets/0011-ios-smoke-test-notes.md).
- [Monetization decision](tickets/0010-monetization.md) — **game stays free; one optional voluntary channel: itch.io pay-what-you-want.** Chosen for portfolio + learning (money incidental). **Ads, premium, IAP all rejected** — ads/IAP would reopen [0006](tickets/0006-economy-upgrades.md)'s no-cash-sink economy and ads collide with the iOS silent-switch mute; IAP needs store rails + accounts the web build doesn't have. **Distribution = hybrid, GitHub Pages canonical** (keeps the installable PWA that protects [0009](tickets/0009-save-system.md)'s iOS save durability) + **itch.io PWYW page** as storefront/devlog/support surface (its own tip rails → no Ko-fi). Touches the build in exactly one place: a quiet "♥ Support / also on itch.io" link on the title/hub screen (never mid-run). **Reopens nothing by design** — economy, save seam, and iOS findings all untouched; native store IAP stays out of scope. [Decision note](assets/0010-monetization.md).
- [Meta-progression & retention](tickets/0012-meta-progression.md) — **persistent mine + ratchet affirmed as the retention engine; one thin honorific layer on top: the "Miner's Log"** — **8 lifetime stats + 14 hidden-until-earned milestones** (Depth 5 / Wealth 4 / Survival 5, incl. a first-lost-run rite-of-passage badge), every one **pinned to an event the game already detects**. **Rewards are honorific-only (hard line** — any material payout = shadow currency = reopens [0006](tickets/0006-economy-upgrades.md)); celebration reuses [0008](tickets/0008-art-audio.md)'s shake+flash + a one-line miner-voiced banner, never a mid-run modal. Surfaces as **one Log screen behind one new hub button** (title screen + [0010](tickets/0010-monetization.md)'s corner untouched; unearned = "???"). Persists as **two [0009](tickets/0009-save-system.md) Dictionary fields** (`stats` 8 int counters, `milestones` string-id map; stat-derivable badges self-heal at load). **Daily hooks rejected on the record** (nothing legal to pay out, streak-guilt is anti-tonal, the shaft *is* the daily hook). [Design note](assets/0012-meta-progression.md).
- [Tutorial & onboarding](tickets/0013-tutorial-onboarding.md) — **onboarding = two text lines, one gauge behaviour, two nudges — zero modals, nothing gates play, two save fields.** Controls ([0004](tickets/0004-dig-feel-controls.md)) get one **ghost line** on first descent, self-dismissing on first dig, first-run *derived* from an empty dug-delta (no flag). Round-trip fuel ([0003](tickets/0003-core-loop.md)) is taught diegetically: the gauge pulse pinned **round-trip aware** (fires when fuel ≈ ascent cost from current depth, threshold an `@export` knob) + a permanent death-reason line on the run-lost screen — the cheap first lesson does the rest. **Add-to-Home-Screen** ([0009](tickets/0009-save-system.md)) = temporary callout on a new permanent **💾 save-safety corner** in the hub (install how-to + save export/import's forever home), triggered by first sell-or-run-lost, one re-show after a later run lost (`nudges.a2hs_dismissed` 0–2), suppressed when standalone. Silent-switch ([0008](tickets/0008-art-audio.md)/[0011](tickets/0011-ios-smoke-test.md)) = first-session-only static caption on tap-to-start (`nudges.audio_hint_shown`). **Final hub census for [0014](tickets/0014-final-spec-assembly.md): 4 actions + Log + ♥ + 💾.** [Design note](assets/0013-tutorial-onboarding.md).
- [Final spec assembly](tickets/0014-final-spec-assembly.md) — **the destination reached: THE MAP IS COMPLETE — no open tickets remain.** All of 0001–0013 compiled into **one design + technical spec** ([Gem Miner — Final Design & Technical Specification](assets/0014-final-spec.md)): Design / Technical / Release / Confirm-during-build + a master knob appendix. Cross-check found **no contradictions**; every shared knob stated once as a reconciled contract (drill-time band 0005×0006, hazard calibration 0006×0007, darkness 0003×0005×0006×0007, hub census, save envelope 0009+0012+0013, memory budget 0002×0003×0005). The two fog patches ride in the spec as **confirm-during-build** items (§16 memory profiling with 0011's instrumentation prerequisite, §17 playtesting plan) — execution-phase work, not open decisions. Open Godot 4.3+ and build.

## Not yet specified

> **Carried by the final spec.** With [0014](tickets/0014-final-spec-assembly.md)
> closed, both entries below are held as explicit **confirm-during-build**
> items in the [final spec](assets/0014-final-spec.md) (§16 on-device memory
> profiling, §17 playtesting plan) — execution-phase work gated on a build
> existing, with their known prerequisites attached, not open decisions. The
